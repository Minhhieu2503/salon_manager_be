...
import com.example.salonmanager.entity.Combo;
...
import com.example.salonmanager.repository.ComboRepo;
import com.example.salonmanager.repository.EmployeeRepo;
import com.example.salonmanager.repository.ServiceRepo;
import com.example.salonmanager.request.ChatState;
...
//    }
    private String handleServiceSelection(ChatState state, Customer customer) {
        List<com.example.salonmanager.entity.Service> services = serviceRepository.findAllServices();
        if (services.isEmpty()) {
            return "Hiện chưa có dịch vụ nào.";
        }
        ...
        for (int i = 0; i < services.size(); i++) {
            com.example.salonmanager.entity.Service service = services.get(i);
            response.append(i + 1)
                    .append(". ")
    ...
    private String handleComboSelection(ChatState state, Customer customer, String message) {
        List<com.example.salonmanager.entity.Service> services = state.getAvailableServices();

        Optional<com.example.salonmanager.entity.Service> serviceOpt = serviceRepository.findByName(message);
    ...
    private String handleEmployeeSelection(ChatState state, Customer customer, String message) {
        ...
        Optional<com.example.salonmanager.entity.Service> serviceOpt = serviceRepository.findByName(state.getServiceChosen());
