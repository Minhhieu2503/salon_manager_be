...
public class OrderServiceImpl implements OrderService{
    ...
    public String bookingScheduleHaircut(OrderScheduleHaircutRequest request) throws LoginException, OrderException {
        ...
        if (!(authentication instanceof AnonymousAuthenticationToken)){
            try {
                ...
                Set<com.example.salonmanager.entity.Service> services = new HashSet<>();
                ...
                if (combo != null){
                    for (com.example.salonmanager.entity.Service service : services) {
    ...
    public void updateBookingStatus(Integer bookingId, Integer status) {
        ...
        if (order.getPaymentType() == Orders.PaymentType.CASH && order.getPaymentStatus() == Orders.PaymentStatus.PENDING && status == 2) {
            order.setPaymentStatus(Orders.PaymentStatus.PAID);
            order.setStatus(2); // Đã thanh toán, hoàn thành
